...
void Particles3D::uniform_background(EMfields3D * EMf)
{
...
*/
void Particles3D::constantVelocity(double vel, int dim, EMfields3D * EMf) {
  switch (dim) {
    case 0:
...
void Particles3D::alt_maxwellian(EMfields3D * EMf) {
  eprintf("unimplemented");
}

#ifdef BATSRUS
/** Maxellian random velocity and uniform spatial distribution */
void Particles3D::MaxwellianFromFluid(EMfields3D* EMf,Collective *col, int is){
...
/** Maxellian random velocity and uniform spatial distribution */
void Particles3D::maxwellian(EMfields3D * EMf)
{
...
void Particles3D::maxwellianNullPoints(EMfields3D * EMf)
{
	/* initialize random generator with different seed on different processor */
	srand(vct->getCartesian_rank()+2);
...
void Particles3D::maxwellianDoubleHarris(EMfields3D * EMf)
{
  /* initialize random generator with different seed on different processor */
  srand(vct->getCartesian_rank() + 2);
...
void Particles3D::pitch_angle_energy(EMfields3D * EMf) {
...
void Particles3D::force_free(EMfields3D * EMf)
{
...
/** 
  Injects cometary particles.
  The number of particles added at each time step is proportional to the OutgassingRate / r^2.
*/
void Particles3D::injectCometaryParticles(Field * EMf,double R, double x_center, double y_center, double z_center) 
{
    srand(vct->getCartesian_rank() + time(0) + ns);
    ...
    double dist_sq = -1.0;
    // Outgassing rate from the inputfile
    double OutgassingRate = col->get_OutgassingRate() * npcelx * npcely * npcelz;
    // Number of particles to add at the given cell
    double Ninject = -1.0;
    // The radius at which the number of produced particles should be fixed, as production rate is ~1/r^2.
    double InternalRadiusSquared = 0.05*0.05;
    // The outgassing inside the sphere
    double NinjectInside = OutgassingRate / InternalRadiusSquared;
    int Ninject_int = -1;
    int nop_injected=0;
    //TODO: pi is initialized in 100 places in iPIC3D...
    double FourPI = 16*atan(1.0);
    
    // Create new particles, identical to the algorithm in the previous iPIC version.
    for (int i=1; i<grid->getNXC()-1;i++)
      for (int j=1; j<grid->getNYC()-1;j++)
        for (int k=1; k<grid->getNZC()-1;k++) 
        {
          // Get the distance of the cell with respect to the comet center
          dist_sq = (grid->getXC(i,j,k) - x_center)*(grid->getXC(i,j,k) - x_center) + (grid->getYC(i,j,k) - y_center)*(grid->getYC(i,j,k) - y_center)+(grid->getZC(i,j,k) - z_center)*(grid->getZC(i,j,k) - z_center);

          // Get the number of electrons to be injected dependent on the distance to the comet center
	  // sphere (1) or full domain (2)
	  switch(2)
	    {
	    case 1:
	      // Only inject particles in a sphere with fixed radius
	      if (dist_sq < (18.0*18.0))
		{
		  // Ninject = npcx*npcy*npcz *  (dt_real*Nu_ion_1AU)  (1/d_sun)*(1/d_sun) * (Q_out/(4*pi*V_n*dp*dp*np))
		  if (dist_sq < InternalRadiusSquared) 
		    Ninject = NinjectInside;
		  else 
		    Ninject = OutgassingRate / dist_sq;
 
		  //Now generate the electrons                    
		  ...
		  for (int jj = 0; jj < Ninject_int; jj++)
		    {
		      ...
		      //TODO: remove the factor of 5?
		      q = qom / (fabs(qom)*FourPI*npcel*invVOL);
		      create_new_particle(u,v,w,q,x,y,z);
		      nop_injected++;
		    }
		}
	    case 2:
	      // Inject particles everywhere except boundary layer?
	      if (grid->getXC(i,j,k) > 10*dx && grid->getXC(i,j,k) < Lx-10*dx && grid->getYC(i,j,k) > 10*dy && grid->getYC(i,j,k) < Ly-10*dy && grid->getZC(i,j,k) > 10*dz && grid->getZC(i,j,k) < Lz-10*dz)
		{
		  // Ninject = npcx*npcy*npcz *  (dt_real*Nu_ion_1AU)  (1/d_sun)*(1/d_sun) * (Q_out/(4*pi*V_n*dp*dp*np))
		  if (dist_sq < InternalRadiusSquared) 
		    Ninject = NinjectInside;
...
/** explicit mover */
void Particles3D::mover_explicit(EMfields3D * EMf) {
  eprintf("unimplemented");
}
//
...
void Particles3D::mover_PC(EMfields3D * EMf) {
#pragma omp parallel
{
...
void Particles3D::mover_PC_AoS(EMfields3D * EMf)
{
	#pragma omp parallel
	{
...
void Particles3D::mover_PC_AoS_Relativistic(EMfields3D * EMf)
{
...
#pragma omp parallel
{
  ...
  for (pidx = 0; pidx < nop; pidx++) {
	  ...
	  for(int ii=0; c<8; c++)
	  {
		Bxl += weights[ii] * field_components[ii][0];
		Byl += weights[ii] * field_components[ii][1];
		Bzl += weights[ii] * field_components[ii][2];
	  }
...
void Particles3D::mover_PC_AoS_vec_intr(EMfields3D * EMf)
{
 #ifndef __MIC__
	   if (MPIdata::get_rank() == 0)
...
void Particles3D::mover_PC_AoS_vec(EMfields3D * EMf)
{
#pragma omp parallel
{
  convertParticlesToAoS();
...
//
//void Particles3D::mover_PC_AoS_vec_onesort(EMfields3D * EMf)
//{
//  convertParticlesToAoS();
//  #pragma omp master
//  if (vct->getCartesian_rank() == 0) {
...
///** mover with a Predictor-Corrector scheme */
//void Particles3D::mover_PC_vectorized(EMfields3D * EMf)
//{
//  convertParticlesToSoA();
//  assert_eq(nxc,nxn-1);
//  assert_eq(nyc,nyn-1);
...
int Particles3D::mover_relativistic(EMfields3D * EMf)
{
...
double Particles3D::delta_f(double u, double v, double w, double x, double y, double kx, double ky, double omega_re, double omega_i, double Ex_mod, double Ex_phase, double Ey_mod, double Ey_phase, double Ez_mod, double Ez_phase, double theta, EMfields3D * EMf) {
