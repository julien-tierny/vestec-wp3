...
int c_Solver::Init(int argc, char **argv) {
...
#endif
  else if ((col->getCase()=="Dipole") || (col->getCase()=="Comet"))    		EMf->initDipole();
  else if (col->getCase()=="Dipole2D")  		EMf->initDipole2D();
  else if (col->getCase()=="NullPoints")             	EMf->initNullPoints();
  else if (col->getCase()=="Shock")             	EMf->initShock();
  else if ((col->getCase()).find("TaylorGreen") == 0) 
  {
    // Different field configurations possible in TaylorGreen
    string case_tg = (col->getCase()).substr(11);
    EMf->initTaylorGreen(std::atoi(case_tg.c_str()), first_cycle);
    if (myrank==0) {
      cout << " =========================================================== " << endl;
      cout << "      Initializing TG from cycle " << first_cycle << endl;
      cout << " =========================================================== " << endl;
    }
  }
  else if ((col->getCase()).find("Politano") == 0) 
  {
    // Different field configurations possible in TaylorGreen
    string case_p = (col->getCase()).substr(8);
    EMf->initPolitano(std::atoi(case_p.c_str()));
  }
  ...
  if (restart_status == 0) {
    for (int i = 0; i < ns; i++)
    {
      ...
      else if ((col->getCase()).find("TaylorGreen") == 0)           part[i].maxwellianNullPoints(EMf); // Flow is initiated from the current prescribed on the grid.
      else if (col->getCase()=="Politano")       	part[i].maxwellianNullPoints(EMf);
...
bool c_Solver::ParticlesMover()
{
  // move all species of particles
  {
    ...
    {
      if ((col->getCase()=="Comet") && ((i==2)||(i==3))) part[i].injectCometaryParticles(EMf,col->getL_square(),col->getx_center(),col->gety_center(),col->getz_center());
...
/** Gathers the number of particles on each process and 
 * Returns the total number of simuation particles. */
long long int c_Solver::get_NOPtotal()
{
  long long nop = 0;

  for (int i = 0; i < ns; i++)
    nop += (long long) part[i].getNOP();

  //TODO: shall there be MPI_Barrier here?
  MPI_Allreduce(MPI_IN_PLACE, &nop, 1, MPI_LONG_LONG, MPI_SUM, MPIdata::get_PicGlobalComm());
