...
/** Compute and return total compute time for each main task. 
 * The return is done via cycle_times argument.
*/
void TimeTasks::get_cycle_times(double* tskdur, const char* reduce_mode, std::map<const char*, double> &cycle_times)
{
    ...
    for(int e=FIELDS; e<=MOMENTS; e++)
    {
      ...
      allredtot += allred[e];
      sndrcvtot += sndrcv[e];
      cycle_times[get_taskname(e)] = comput;
    }

    cycle_times["total"] = computtot;
}
...
/** Collect average times from all processes. */
void TimeTasks::get_cycle_times(const char* reduce_mode, std::map<const char*, double> &cycle_times)
{
  MPI_Op mpi_op;
  if(!strcmp(reduce_mode,"max"))
    mpi_op = MPI_MAX;
  ...
  get_cycle_times(reported_task_duration, reduce_mode, cycle_times);
}
