...
/**
  Expand Taylor-Green simulation twice in each dimension as suggested by Sergio Servidio

  arr - a large (whole simulation domain-wide array with quantity read from restart
  field - a reference to one of EMField3D's fields, e.g., Bx, By, Bz, Jx, Jy, Jz...
*/
void EMfields3D::expandTaylorGreenField(arr3_double arr, arr3_double field)
{
  // My coordinates in the Cartesian processor topology
  const VirtualTopology3D *vct = &get_vct();
  const Collective *col = &get_col();

  // Number of nodes in the whole computational domain. Without ghost nodes.
  const int nxn_tot2 = col->getNxc() / 2 + 1;
  const int nyn_tot2 = col->getNyc() / 2 + 1;
  const int nzn_tot2 = col->getNzc() / 2 + 1;

  // Indices of the lower left node of my subdomain
  int i_start = vct->getCoordinates(0) * (col->getNxc() / col->getXLEN());
  int j_start = vct->getCoordinates(1) * (col->getNyc() / col->getYLEN());
  int k_start = vct->getCoordinates(2) * (col->getNzc() / col->getZLEN());

  // Assign values on-by-one
  for (int i=0; i < nxc-1; i++)
  for (int j=0; j < nyc-1; j++)
  for (int k=0; k < nzc-1; k++)
  {
    int i_arr = i_start + i;
    i_arr = i_arr < nxn_tot2 ? i_arr * 2 : (i_arr - nxn_tot2) * 2;

    int j_arr = j_start + j;
    j_arr = j_arr < nyn_tot2 ? j_arr * 2 : (j_arr - nyn_tot2) * 2;
    
    int k_arr = k_start + k;
    k_arr = k_arr < nzn_tot2 ? k_arr * 2 : (k_arr - nzn_tot2) * 2;

    field[i+1][j+1][k+1] = arr[i_arr][j_arr][k_arr];
  }
}
...
/**
  ...
  is - specie index.
*/
void EMfields3D::expandTaylorGreenMoment(arr3_double arr, arr4_double field, int is)
{
  // My coordinates in the Cartesian processor topology
  const VirtualTopology3D *vct = &get_vct();
  const Collective *col = &get_col();
  ...
  for (int k=0; k < nzc-1; k++)
  {
    ...
    field[is][i+1][j+1][k+1] = arr[i_arr][j_arr][k_arr];
  }
}

/** 
  Taylor-Green flow with superimposed magnetic field as defined in E.Lee at al. (2010) 
  Three possible cases of MF are possible: tg_case = 1, 2, 3.
  When tg_case == 0 the field is zero.
*/
void EMfields3D::initTaylorGreen(int tg_case, int &restarting_from_cycle)
{
  const VirtualTopology3D *vct = &get_vct();
  const Grid *grid = &get_grid();
  const Collective *col = &get_col();
  ...
  if (restart1 == 0)
  {
    ...
    {
      cout << "----------------------------------------" << endl;
      cout << "       Initialize Taylor-Green flow case " << tg_case << endl;
      cout << "----------------------------------------" << endl;
      cout << "B0 = " << B0x << endl;
      cout << "u0 = " << ue0 << endl;
      for (int i=0; i < ns; i++)
    ...
    if (tg_case != 4)
    {
      ...
      for (int k=0; k < nzn; k++)
      {
        // Velocity and density is the same in different magnetic configurations. Just the Taylor-Green flow.
        for (int is=0; is < ns; is++) 
        {
          rhons[is][i][j][k] = rhoINIT[is]/FourPI;             
          // The flow will be initialized from currents
          Jxs[is][i][j][k] = ue0 * rhons[is][i][j][k] * sin(2.*M_PI*grid->getXN(i,j,k)/Lx) * cos(2.*M_PI*grid->getYN(i,j,k)/Ly) * cos(2.*M_PI*grid->getZN(i,j,k)/Lz); 
          Jys[is][i][j][k] = -ue0 * rhons[is][i][j][k] * cos(2.*M_PI*grid->getXN(i,j,k)/Lx) * sin(2.*M_PI*grid->getYN(i,j,k)/Ly) * cos(2.*M_PI*grid->getZN(i,j,k)/Lz);
        ...
        else if (tg_case == 2)
        {
          ...
          Bxn[i][j][k] =  B0x * sin(4.*M_PI*grid->getXN(i,j,k)/Lx) * cos(4.*M_PI*grid->getYN(i,j,k)/Ly) * cos(4.*M_PI*grid->getZN(i,j,k)/Lz);
          Byn[i][j][k] =  B0x * cos(4.*M_PI*grid->getXN(i,j,k)/Lx) * sin(4.*M_PI*grid->getYN(i,j,k)/Ly) * cos(4.*M_PI*grid->getZN(i,j,k)/Lz);
          Bzn[i][j][k] =  -2. * B0x  * cos(4.*M_PI*grid->getXN(i,j,k)/Lx) * cos(4.*M_PI*grid->getYN(i,j,k)/Ly) * sin(4.*M_PI*grid->getZN(i,j,k)/Lz);
        }
        else if (tg_case == 3)
        {
          ...
          Bxn[i][j][k] = B0x * cos(4.*M_PI*grid->getXN(i,j,k)/Lx) * sin(4.*M_PI*grid->getYN(i,j,k)/Ly) * sin(4.*M_PI*grid->getZN(i,j,k)/Lz);
          Byn[i][j][k] = -B0x * sin(4.*M_PI*grid->getXN(i,j,k)/Lx) * cos(4.*M_PI*grid->getYN(i,j,k)/Ly) * sin(4.*M_PI*grid->getZN(i,j,k)/Lz);
          Bzn[i][j][k] = 0.;
        }
        else
        {
      ...
      // Init cell quantities. Why not interpolate?
      for (int i=0; i <nxc; i++)
      for (int j=0; j <nyc; j++)
      for (int k=0; k <nzc; k++) 
      {
        ...
        {
          Bxc[i][j][k] = B0x * sin(4.*M_PI*grid->getXC(i,j,k)/Lx) * cos(4.*M_PI*grid->getYC(i,j,k)/Ly) * cos(4.*M_PI*grid->getZC(i,j,k)/Lz);
          Byc[i][j][k] = B0x * cos(4.*M_PI*grid->getXC(i,j,k)/Lx) * sin(4.*M_PI*grid->getYC(i,j,k)/Ly) * cos(4.*M_PI*grid->getZC(i,j,k)/Lz);
          Bzc[i][j][k] = -2. * B0x * cos(4.*M_PI*grid->getXC(i,j,k)/Lx) * cos(4.*M_PI*grid->getYC(i,j,k)/Ly) * sin(4.*M_PI*grid->getZC(i,j,k)/Lz);
        }
        else if (tg_case == 3)
        {
          Bxc[i][j][k] = B0x * cos(4.*M_PI*grid->getXC(i,j,k)/Lx) * sin(4.*M_PI*grid->getYC(i,j,k)/Ly) * sin(4.*M_PI*grid->getZC(i,j,k)/Lz);
          Byc[i][j][k] = -B0x * sin(4.*M_PI*grid->getXC(i,j,k)/Lx) * cos(4.*M_PI*grid->getYC(i,j,k)/Ly) * sin(4.*M_PI*grid->getZC(i,j,k)/Lz);
          Bzc[i][j][k] = 0.;
        }
        else
        {
    ...
    {
      // Case 4. Load from restart and reinterpolate on a larger grid as suggested by Sergio Servidio.

#ifdef NO_HDF5
      eprintf("Taylor-Green restart requires compiling with HDF5");
#else
      // Read arrays one-by-one to save memory
      array3_double field_tot(col->getNxc() + 1, col->getNyc() + 1, col->getNzc() + 1);

      // Fields. Read one array at a time because it is memory-consuming
      restarting_from_cycle = col->read_total_field_restart(vct, grid, field_tot, 0, "Bx");
      restarting_from_cycle += 1;
      expandTaylorGreenField(field_tot, Bxn);
      col->read_total_field_restart(vct, grid, field_tot, 0, "By");
      expandTaylorGreenField(field_tot, Byn);
      col->read_total_field_restart(vct, grid, field_tot, 0, "Bz");
      expandTaylorGreenField(field_tot, Bzn);
      col->read_total_field_restart(vct, grid, field_tot, 0, "Ex");
      expandTaylorGreenField(field_tot, Ex);
      col->read_total_field_restart(vct, grid, field_tot, 0, "Ey");
      expandTaylorGreenField(field_tot, Ey);
      col->read_total_field_restart(vct, grid, field_tot, 0, "Ez");
      expandTaylorGreenField(field_tot, Ez);

      // Moments
      for (int is = 0; is < ns; is++) 
      {
        double q_factor = qom[is]/fabs(qom[is]);
    
        col->read_total_field_restart(vct, grid, field_tot, is, "rho");
        // When initing from scratch, the density fed to initialize particles is rhoINIT/4pi
        expandTaylorGreenMoment(field_tot, rhons, is);
        for (int i=0; i < nxn; i++)
        for (int j=0; j < nyn; j++)
        for (int k=0; k < nzn; k++)
        {
          rhons[is][i][j][k] *= q_factor;
          // The flow will be initialized from currents
          //Jxs[is][i][j][k] = ue0 * rhons[is][i][j][k] * sin(2.*M_PI*grid->getXN(i,j,k)/Lx) * cos(2.*M_PI*grid->getYN(i,j,k)/Ly) * cos(2.*M_PI*grid->getZN(i,j,k)/Lz); 
          //Jys[is][i][j][k] = -ue0 * rhons[is][i][j][k] * cos(2.*M_PI*grid->getXN(i,j,k)/Lx) * sin(2.*M_PI*grid->getYN(i,j,k)/Ly) * cos(2.*M_PI*grid->getZN(i,j,k)/Lz);
          //Jzs[is][i][j][k] = 0.; // Z velocity is zero
        ...
        col->read_total_field_restart(vct, grid, field_tot, is, "Jx");
        expandTaylorGreenMoment(field_tot, Jxs, is);        
        col->read_total_field_restart(vct, grid, field_tot, is, "Jy");
        expandTaylorGreenMoment(field_tot, Jys, is);        
        col->read_total_field_restart(vct, grid, field_tot, is, "Jz");
        expandTaylorGreenMoment(field_tot, Jzs, is);       
      }
      ...
      {
        double ***moment0 = convert_to_arr3(rhons[is]);
        double ***moment1 = convert_to_arr3(Jxs  [is]);
        double ***moment2 = convert_to_arr3(Jys  [is]);
        double ***moment3 = convert_to_arr3(Jzs  [is]);
        communicateNode_P(nxn, nyn, nzn, moment0, vct, this);
        communicateNode_P(nxn, nyn, nzn, moment1, vct, this);
        communicateNode_P(nxn, nyn, nzn, moment2, vct, this);
        communicateNode_P(nxn, nyn, nzn, moment3, vct, this);
    ...
    // Interpolate density to the cell centers
    for (int is=0 ; is<ns; is++)
      grid->interpN2C(rhocs,is,rhons);
  } 
  else 
...
/** 
  Magnetic nulls + turbulence based on [Politano, Pouquet & Sulem, Physics of Plasmas 2, 2931 (1995)]
  ...
  pcase
    ...
    2 - ?
*/
void EMfields3D::initPolitano(int pcase)
{
  const VirtualTopology3D *vct = &get_vct();
  const Grid *grid = &get_grid();
  const Collective *col = &get_col();
  
  // alpha defines correlation coefficient in Politano's paper (alpha = 0.8).
  double alpha = 0.8;
  ...
  if (restart1 == 0)
  {
    ...
    {
      cout << "----------------------------------------" << endl;
      cout << "  Initialize multiple nulls (Politano), case " << pcase << endl;
      cout << "----------------------------------------" << endl;
      cout << "B0 = " << B0x << endl;
      cout << "u0 = " << ue0 << endl;
      for (int i=0; i < ns; i++)
    ...
    for (int k=0; k < nzn; k++)
    {
      // Node quantities. Velocity will be initialized from currents
      for (int is=0; is < ns; is++) 
      {
        rhons[is][i][j][k] = rhoINIT[is]/FourPI;             
        // The flow will be initialized from currents. In the original paper, velocity represents the Orszag-Tang vortex.
        Jxs[is][i][j][k] = -2. * ue0 * rhons[is][i][j][k] * sin(2*M_PI*(grid->getYN(i,j,k)/Ly)); 
        Jys[is][i][j][k] = 2.  * ue0 * rhons[is][i][j][k] * sin(2*M_PI*(grid->getXN(i,j,k)/Lx));
        Jzs[is][i][j][k] = 0.; // Z velocity is zero
      }
      ...
      // Different magnetic field configuration? With and without random phases?
      //if (pcase == 0)
      {
        Bxn[i][j][k] = B0x * (-2. * sin(4*M_PI*(grid->getYN(i,j,k)/Ly)) + sin(2*M_PI*(grid->getZN(i,j,k)/Lz)));
        Byn[i][j][k] = B0x * (2. * sin(2*M_PI*(grid->getXN(i,j,k)/Lx)) + sin(2*M_PI*(grid->getZN(i,j,k)/Lz)));
        Bzn[i][j][k] = B0x * (sin(2*M_PI*(grid->getXN(i,j,k)/Lx)) + sin(2*M_PI*(grid->getYN(i,j,k)/Ly)));;
      }
      //else if (pcase == 1)
    }
    ...
    for (int k=0; k <nzc; k++) 
    {
      //if (pcase == 0)
      {
        Bxc[i][j][k] = B0x * (-2. * sin(4*M_PI*(grid->getYC(i,j,k)/Ly)) + sin(2*M_PI*(grid->getZC(i,j,k)/Lz)));
        Byc[i][j][k] = B0x * (2. * sin(2*M_PI*(grid->getXC(i,j,k)/Lx)) + sin(2*M_PI*(grid->getZC(i,j,k)/Lz)));
        Bzc[i][j][k] = B0x * (sin(2*M_PI*(grid->getXC(i,j,k)/Lx)) + sin(2*M_PI*(grid->getYC(i,j,k)/Ly)));;
      }
      //else if (pcase == 1)
    }
...
/*! initialize "Shock" case, but with unifrom init B field with no Perturbation */
void EMfields3D::initShock()
{
    const Collective *col = &get_col();
    const VirtualTopology3D *vct = &get_vct();
    const Grid *grid = &get_grid();
    if (restart1 == 0) {
        ...
        if (get_vct().getCartesian_rank() == 0) {
            cout << "----------------------------------------------" << endl;
            cout << "Initialize Shock case, but with unifrom init B field with no Perturbation " << endl;
            cout << "----------------------------------------------" << endl;
            cout << "NOTE THAT SPECIE 0 VELOCITY provides convective E field at t=0; change if needed in EMfields3D::initShock()"<<endl;
            cout << "B0x                              = " << B0x << endl;
            cout << "B0y                              = " << B0y << endl;
            cout << "B0z                              = " << B0z << endl;
            //cout << "Delta (current sheet thickness) = " << delta << endl;
        ...
        for (int i = 0; i < nxn; i++)
            for (int j = 0; j < nyn; j++)
                for (int k = 0; k < nzn; k++) {
                    ...
                    // electric field: just convection. Assume that e (specie 0) velocity gives convective E field at t=0
                   
                    Ex[i][j][k] = (col->getW0(0))*B0y - (col->getV0(0))*B0z;
                    Ey[i][j][k] = (col->getU0(0))*B0z - (col->getW0(0))*B0x;
                    Ez[i][j][k] = (col->getV0(0))*B0x - (col->getU0(0))*B0y;
                    // Magnetic field
                    Bxn[i][j][k] = B0x;// * tanh((grid->getYN(i, j, k) - Ly / 2) / delta);
                    Byn[i][j][k] = B0y;
                    // guide field
  ...
  int nx, int ny, int nz)
{
  ...
  */
  ...
  /// Andrey's way of setting open BCs
  if(vct->getXleft_neighbor()==MPI_PROC_NULL && bcEMfaceXleft ==2 && nx>10) {
    for (int j=0; j < ny;j++)
      for (int k=0; k < nz;k++)
      {
