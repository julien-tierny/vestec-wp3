...
#endif
{
  ...
  public:
    ...
    /// Read field from restart for the current process
    ...
    /// Reads the specific array (quantity) and combines it for all processes
    int read_total_field_restart(const VCtopology3D* vct, const Grid* grid, arr3_double arr, int is, const char* quantity)const;

    /// Read particles from the restart for the current process
    ...
    double get_OutgassingRate()const{ return (OutgassingRate); }
  ...
  private:
    ...
    double L_square;
    /** 
      Normalized outgassing rate for cometary simulations.
      The normalized outgassing rate should be computed outside using the corresponding python/MATLAB script, and be compatible with [Hansen et al., MNRAS 462, S491 (2016)]. 
      The outgassing rate defines the number of superparticles produced at each timestep. 
        dn = Q * dt / r^2,
      where r^2 = (x_c - x_comet)^2 + (y_c - y_comet)^2 + (z_c - z_comet)^2.

      The value 1.223e-4 corresponds to 2 AU outgassing of 1e27 s^-1
    */
    double OutgassingRate;
