...
class Particles3D:public Particles3Dcomm {
  ...
  public:
    ...
    void uniform_background(EMfields3D * EMf);
    /** Initialize particles with a constant velocity in dim direction. Depending on the value of dim:
      ...
      */
    void constantVelocity(double vel, int dim, EMfields3D * EMf);
    /** Initial condition: uniform in space and maxwellian in velocity */
    void maxwellian(EMfields3D * EMf);
    /** Initial condition: uniform in space and maxwellian in velocity with velocity from Null Point currents */
    void maxwellianNullPoints(EMfields3D * EMf);
    /** Maxellian velocity from currents and uniform spatial distribution */
    void maxwellianDoubleHarris(EMfields3D * EMf);
    /** pitch_angle_energy initialization (Assume B on z only) for test particles */
    void pitch_angle_energy(EMfields3D * EMf);
    /** Force Free initialization (JxB=0) for particles */
    void force_free(EMfields3D * EMf);
    /** Initial condition: uniform in space and maxwellian in velocity */
    void alt_maxwellian(EMfields3D * EMf);
    /** Linear_perturbation */
    //void linear_perturbation(double deltaBX, double kx, double ky, double theta, double omega_r, double omega_i, double Ex_mod, double Ex_phase, double Ey_mod, double Ey_phase, double Ez_mod, double Ez_phase, double Bx_mod, double Bx_phase, double By_mod, double By_phase, double Bz_mod, double Bz_phase, EMfields3D * EMf);
    ...
    double delta_f(double u, double v, double w, double x, double y, double kx, double ky, double omega_re, double omega_i, double Ex_ampl, double Ex_phase, double Ey_ampl, double Ey_phase, double Ez_ampl, double Ez_phase, double theta, EMfields3D * EMf);
    ...
    void mover_explicit(EMfields3D * EMf);
    /** mover with a Predictor-Corrector Scheme */
    void mover_PC(EMfields3D * EMf);
    /** array-of-structs version of mover_PC */
    void mover_PC_AoS(EMfields3D * EMf);
    /** Relativistic array-of-structs version of mover_PC with adaptive Subcycling and PC*/
    void mover_PC_AoS_Relativistic(EMfields3D * EMf);
    /* vectorized version of previous */
    void mover_PC_AoS_vec(EMfields3D * EMf);
    /* mic particle mover */
    void mover_PC_AoS_vec_intr(EMfields3D * EMf);
    /* this computes garbage */
    void mover_PC_AoS_vec_onesort(EMfields3D * EMf);
    /** vectorized version of mover_PC **/
    void mover_PC_vectorized(EMfields3D * EMf);
    /** relativistic mover with a Predictor-Corrector scheme */
    int mover_relativistic(EMfields3D * EMf);
   private:
   ...
   public:
    ...
    /** Injects new superparticles at each timestep depending on the outgassing rate read from the inputfile. */
    void injectCometaryParticles(Field * EMf,double R, double x_center, double y_center, double z_center);

#ifdef BATSRUS
    /*! Initial condition: given a fluid model (BATSRUS) */
    void MaxwellianFromFluid(EMfields3D* EMf,Collective *col, int is);
